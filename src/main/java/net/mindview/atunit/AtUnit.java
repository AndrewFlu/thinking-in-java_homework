...
public class AtUnit implements ProcessFiles.Strategy {
    ...
    public static void main(String[] args) {
        ClassLoader.getSystemClassLoader().setDefaultAssertionStatus(true); // включить  assert
        ...
        if (testMethods.size() > 0) {
            if (creator == null)
                try {
                    ...
                    // Синтезированный конструктор по умолчанию; ОК
                }
        ...
        for (Method m : testMethods) {
            ...
            try {
                ...
                try {
                    ...
                    // Фактическое исключение внутри ex
                    print(ex.getCause());
                }
    ...
    private static Method checkForCreatorMethod(Method m) {
        ...
        if (!m.getReturnType().equals(testClass))
            throw new RuntimeException("@TestObjectCreate must return instance of Class to bew tested");
